...
PRINT '=== Actualizar vista vw_cn_clientes con columnas VENDEDOR ===';
GO

IF EXISTS (SELECT 1 FROM sys.views WHERE name = 'vw_cn_clientes' AND schema_id = SCHEMA_ID('api'))
BEGIN
    EXEC('
    CREATE OR ALTER VIEW api.vw_cn_clientes
    AS
    SELECT
        c.company_id,
        c.customer_id,
    ...
    ');
    PRINT 'OK: Vista api.vw_cn_clientes actualizada';
END
ELSE
BEGIN
    PRINT 'WARNING: Vista api.vw_cn_clientes no encontrada';
END
GO

PRINT '=== Verificacion: columnas de la vista ===';
SELECT COLUMN_NAME 
FROM INFORMATION_SCHEMA.COLUMNS 
WHERE TABLE_SCHEMA = 'api' AND TABLE_NAME = 'vw_cn_clientes'
